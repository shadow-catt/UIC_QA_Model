...
def obtain_stop_word(path):
    ...
    """
    stop_words = [line.strip() for line in open(path, mode='rb').readlines()]
    stop_words.extend([""," "])
    return stop_words
...
def TF_IDF_prepared(data_file_path, stopword_file_path):
    ...
    return question_list, question_token_list, answer_list, stop_words

def TF_IDF_reply(query, question_list, question_token_list, answer_list, stop_words, topk_TFIDF,threshold_TFIDF):
    # 对查询的问题进行处理
    query_processed = cn_stop_word_rm(query, stop_words)

    # 得到问题（答案）所对应的行索引
    if_valid, topk_idx_TF, score_TF = similarity_cn(question_token_list, query_processed, topk_TFIDF, threshold_TFIDF, all_score_without_rank=0)

    # 返回最相似问题
    # return question_list[topk_idx_TF[0]]
...
def TF_IDF_QA():
    ...
    threshold_TFIDF = 0.7
    # data prepared
    question_list, question_token_list, answer_list, stop_words = TF_IDF_prepared(data_file_path, stopword_file_path)
    print("准备完毕")
    # obtain the question
    while True:
        # 读取数据
        ...
        TF_IDF_reply(query, question_list, question_token_list, answer_list, stop_words, topk_TFIDF, threshold_TFIDF)
