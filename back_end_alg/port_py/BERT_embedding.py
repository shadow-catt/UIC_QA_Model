...
def Bert_em_reply(query, tokenizer, model, question_list, answer_list, doc_vecs, topk=5, threshold=0.95):
    ...
    # 匹配
    if_vaild, Bert_emb_topk_idx, Bert_emb_each_score = get_similar_q_id(query_vec, doc_vecs, tokenizer, model, topk,
                                                                        threshold)
    # return question_list[Bert_emb_topk_idx[0]]
    # 返回对应的答案
    return answer_list[Bert_emb_topk_idx[0]]

    # # 返回最相似的问题
    # print('top %d questions similar to "%s"' % (topk, colored(query, 'green')))
    # for idx in Bert_emb_topk_idx:
    #     print(
    #         '&gt; %s\t%s' % (colored('%.4f' % Bert_emb_each_score[idx], 'cyan'), colored(question_list[idx], 'yellow')))
...
def Bert_em_QA():
    ...
    while True:
        ...
        # get the reply
        Bert_em_reply(query, tokenizer, model, question_list, answer_list, doc_vecs, Bert_emb_topk, Bert_emb_threshold)
...
#
# @app.route("/")
# @app.route("/bert/query", methods=['POST', 'GET'])
# def query_answer():
#     try:
...
#         if query:
#             result = Bert_em_reply(query, tokenizer, model, question_list, answer_list, doc_vecs, Bert_emb_topk, Bert_emb_threshold)
#             return jsonify({"answer": result})
#         else:
#             return jsonify({'status': False, 'msg': '请输入您的问题哦！'}), 400
#     except ValueError as ve:
#         return jsonify({'status': False, 'msg': '入参格式不正确！'}), 400
#
#
# if __name__ == "__main__":
#     app.run()
